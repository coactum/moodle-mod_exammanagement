...
function xmldb_exammanagement_upgrade($oldversion) {
    ...
    if ($oldversion < 2018121500) { // Remove termbased participants table and replacve it with.
        ...
        $table->add_index('plugininstanceid', XMLDB_INDEX_NOTUNIQUE, ['plugininstanceid']);
    ...
    if ($oldversion < 2021022100) { // Add new fields for setting bonus and result user visibility.
        ...
        $table = new xmldb_table('exammanagement_temp_part');
        $fieldold = new xmldb_field('line', XMLDB_TYPE_INTEGER, '10', null, null, null, null);
        $fieldnew = new xmldb_field('line', XMLDB_TYPE_CHAR, '10', null, null, null, null, 'identifier');

        if ($dbman->field_exists($table, $fieldold)) {
            $dbman->change_field_type($table, $fieldnew);
        }
    ...
    if ($oldversion < 2021031300) { // Add new fields for setting bonus and result user visibility.
        ...
        $table = new xmldb_table('exammanagement');
        $fieldold = new xmldb_field('assignmentmode', XMLDB_TYPE_INTEGER, '2', null, null, null, null, 'textfield');
        $fieldnew = new xmldb_field('assignmentmode', XMLDB_TYPE_INTEGER, '3', null, null, null, null, 'textfield');
