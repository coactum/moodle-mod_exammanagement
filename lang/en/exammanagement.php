...
$string['data_deleted'] = 'For data protection reasons, the exam data of all participants in this exam management were deleted three months after completion of the correction phase. This exam management can therefore no longer be used; only the basic data of the exam can still be viewed.';
...
3. the course grades must first be exported and then imported into the exam management. <br><br>
Then, after the results have been checked and the preparations are completed, the grades assigned to the participants matriculation numbers can be exported as a text file.';
...
$string['grading_points'] = 'Grades or grading points';
...
$string['exam_results_overview_grades'] = 'Here the already imported gradings and the grades eventually calculated from them can be viewed.';
$string['complete_preparations'] = 'Complete preparations';
$string['participantsoverview_grades_help'] = 'In this <strong>grading overview</strong>, for all imported participants their grading points and the grades eventually calculated from them can be viewed and edited.<br><br>
...
// Strings for configurepassword.php and form.
...
$string['checkpasswordstr'] = 'A password for this exam management was set by the lecturer. You have to enter it below to gain access.';
...
$string['password_reset_successfull'] = 'The password of the exam management was successfully reset and all lecturers of the {$a->systemname} course were informed about this via {$a->systemname} message.';
$string['password_reset_failed'] = 'Password reset failed due to missing permissions.';
$string['password_reset_request_successfull'] = 'The reset of the password of the exam management was successfully applied. As soon as the password has been reset, you and all other lecturers of the {$a->systemname} course will be informed via {$a->systemname} message.';
...
$string['state_added_to_exam_no_moodle_help'] = 'This participant must log in to the system at least once to view the exam information in his participant view. He also cannot receive group messages sent via the exam management and cannot be added to course groups.';
...
$string['state_nonmoodle_help'] = 'After his import this participant must log in to the system at least once before he can view the exam information in his participant view. He also cannot receive group messages sent via the exam management and cannot be added to course groups before.';
...
$string['mailfooter'] = 'This message was sent via the exam management in {$a->systemname}. You can find all further information under the following link: <br> {$a->categoryname} -> {$a->coursename} -> Prüfungsorganisation -> {$a->name} <br> {$a->url}';

// Strings for assignplaces.php and form.
...
$string['cancel'] = 'Back to exam management';
...
$string['participantslist_names'] = 'Participantslist_Names';
$string['participantslist_places'] = 'Participantslist_Places';
...
// Strings for the exported rsesults.
...
$string['overview_help'] = 'This is the <strong>overview page of the exam management</strong>. Lecturers and / or their staff can see all necessary and helpful work steps for performing an exam. <br><br>
...
The "Configure password" button also allows you to (re)set a password, which must then be entered to access the exam management. With this you can, for example, prevent your student assistants who supervise your courses from accessing the sensitive contents of the exam management. <br><br>
...
Admins can reset the password of the exam management here if lecturers request this. All lecturers of the {$a} course will be informed about this via {$a} message.';
...
By clicking on the button "Reset password" you can revoke the password protection for the exam management.';
...
<ul><li>In order to be able to use the other functions of the {$a} exam management, at least one room must be selected here as exam room. In addition, the selected rooms must offer at least as many seats as participants are to take part in the exam.</li>
...
$string['importdefaultrooms_help'] = 'As a {$a} administrator, you can import a number of <strong>default rooms </strong> here which are after that available to all lecturers as possible rooms when they select their exam rooms. <br><br>
...
All existing default rooms are overwritten by a new import. The information on any deleted or changed rooms is retained in all exam managements in which they are currently used. However, deleted rooms can no longer be selected by instructors as new exam rooms or used for the (re)assignment of seats. Changes to the names or seats of default rooms also become only effective in the individual exam managements after a new assignment of seats.';
...
2. It is also possible to import participants of the {$a} course as exam participants by clicking on the button "Import participants from course". If this option is selected, the exam results can later only be exported in a single result list, a listwise export and a simple reimport of the exam results is then not possible. It is also not possible to "rewrite" participants who have been imported as course participants later by subsequently importing a list with exam participants. To do this, the participant must first be completely deleted.<br><br>
Adding participants is one of the most important steps in the exam management. Only if you see at least one added participant here you will later be able to assign seats, enter exam points or export result documents. Students who have not been added as exam participants (even if they are already enrolled in the {$a} course) also do not have access to the participant view with the exam information and do not receive any notifications send with the messaging function on the overview page of the exam management.<br /> <br />
If you see participants with a corresponding status, then you have imported exam participants that do not have a user account in {$a}. Although these can also be imported from a file, some steps, such as writing a notification, must be done manually for these participants and others (such as viewing the student view for the participants themselves) are completely impossible.<br><br>
...
Finally, the button "Convert to group" can be used to convert individual or all participants listed here into a {$a} group.';
$string['converttogroup_help'] = 'Here selected or all imported exam participants can be converted into an {$a} group. <br /> <br />
To do this, either for the creation of a new {$a} group its name and optionally also description can be entered into the corresponding form fields or an already existing group can be selected. Then all desired participants can be selected in the lower section and afterwards converted into a group by clicking the button "Convert to group".<br /> <br />
Exam participants who are no course participants or do not have a {$a} user account are displayed in a separate section and cannot be added to a group. <br /> <br />
Groups created here can be used throughout the course and all activities just like normal groups. After clicking on a group, for example in the participant overview, it can be edited or changed.';
...
<li><strong>New (no course participant)</strong>: The student belonging to this matriculation number is not part of the {$a} course. He can easily be imported as an exam participant. However, since he cannot view the participant view of the plugin, he must be selected manually by ticking the checkbox in order to exclude the possibility of an error here.</li>
<li><strong>New (without {$a} account)</strong>: The student belonging to this matriculation number does not yet have an account in {$a}. This can happen, for example, if the student has never registered in {$a} before. Although the student can be imported as an exam participant, he or she cannot view the participant view of the exam management and you cannot reach him or her via the notification function of the exam management. Therefore you have to check this student here manually.</li>
...
$string['addcourseparticipants_help'] = 'Here you can import all <strong>{$a}course participants</strong> as exam participants. <br><br>
...
They receive the message immediately after sending it both as a {$a} notification and as an e-mail to their university e-mail account and can thus, for example, simply be made aware of short-term changes (such as changes of the exam times or rooms). <br /> <br />
If you have added participants to the exam who do not yet have a {$a} user account, this will be displayed below. Since these participants will not automatically receive the message written here, you will have to write to them manually by e-mail instead. You can do this, for example, by clicking on the "Write Email" button, which opens your email client and enters the email addresses of the corresponding participants. <br /> <br />
...
1. Exported grades from {$a}: If your students have submitted their exercise sheets via the {$a} assignment activity and these have been corrected and graded there, the exported grades from {$a} should be selected here, since in this way all bonus points for the complete {$a} course can be easily read in. <br>
To do this, the gradings from the {$a} course must first be exported (see <a href="https://docs.moodle.org/en/Grade_export" class="alert-link" target="_blank">here</a>). Then you have to open the exported file once and check in which column the points are entered. The name of the column must then be entered in the field provided in the lower section. <br><br>
2. Individual: If you have not managed your bonus points via the {$a} assignment activity, you can alternatively select the mode "Other". For this you need an Excel file, in which for each participant affected either the email address stored in {$a} or the matriculation number are entered in one and the achieved points in another column in a separate line. The name of both the column containing the user indexes of all students and the column containing all bonus points must then be entered in the corresponding fields in the lower section. <br><br>
...
$string['err_examdata_deleted'] = 'The exam data has already been deleted. It is no longer possible to use the exam management.';
...
$string['security_password_help'] = 'Setting a security password allows you to restrict access to the exam management. Other staff users like student tutors have to enter this passwort before they can access the contents of the exam management.';
...
$string['export_grades_as_exam_results_help'] = 'If this option is activated, the course gradings of all participants can easily be exported in a text document with the exam management. Each grading is assigned to the matriculation number of the respective participant.';
...
$string['exammanagement:addinstance'] = 'Add new exam management';
$string['exammanagement:viewinstance'] = 'View exam management';
$string['exammanagement:viewparticipantspage'] = 'View participants page';
$string['exammanagement:takeexams'] = 'Take exams';
$string['exammanagement:receivegroupmessage'] = 'Receive group messages from exam management';
...
$string['messageprovider:groupmessage'] = 'Group messages for exam management';
$string['messageprovider:passwordresetrequest'] = 'Requests to reset the password of exam managements';
$string['messageprovider:passwordresetmessage'] = 'Confirmation messages of resetting the password of exam management';
...
$string['privacy:metadata:exammanagement:no_data'] = 'No personal data.';
...
$string['enablepasswordresetrequest_help'] = 'As soon as this function has been activated, all teachers in their exam managements can request the reset of the passwords set there by clicking on the corresponding button. If a lecturer has done this, all users with the role "Manager" receive an automatically generated message both as internal notification and forwarded to the e-mail address stored in their profile and can then reset the password by clicking on the link contained in this message. This means that all teachers of the exam management concerned are automatically informed via internal notification and e-mail that the password has been reset and that the contents of the exam management can be accessed again without entering a password. If this function is not activated, users cannot automatically request the password reset in their exam management, but managers and administrators can still reset the password of any exam management.';
...
$string['enableglobalmessage'] = 'Show message when creating an exam management';
$string['enableglobalmessage_help'] = 'If this option is activated, the following global message will be displayed when creating a new exam management.';
$string['globalmessage'] = 'Global message';
$string['globalmessage_help'] = 'Text of the global message that will be displayed when creating a new exam management (if option above is activated).';
$string['enablehelptexts'] = 'Enable intern help texts';
$string['enablehelptexts_help'] = 'If this option is activated, plugin help texts are displayed in all exam managements.';
...
$string['ldap_field_map_username_help'] = 'The name of the field in the LDAP system in which the username of the participant is located must be entered here. This username must match the username of the participant in {$a}. If this field is left empty, the plugin uses the value "field_map_idnumber" from the global settings of the authentication plugin LDAP server ("auth_ldap"). If neither of these two fields is filled in, the LDAP dependent plugin functions cannot be used.';
