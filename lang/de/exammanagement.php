...
Als Administrator können Sie hier auf Wunsch der oder des Lehrenden das Passwort der Prüfungsorganisation zurücksetzen. Alle Lehrenden des {$a}-Kurses werden darüber per {$a}-Nachricht benachrichtigt.';
...
<ul><li>Um die weiteren Funktionen der {$a} Prüfungsorganisation nutzen zu können muss hier mindestens ein Raum als Prüfungsraum ausgewählt werden. Zudem müssen die gewählten Räume mindestens so viele Sitzplätze bieten, wie Teilnehmer/innen an der Prüfung teilnehmen sollen.</li>
...
$string['importdefaultrooms_help'] = 'Hier können Sie als {$a}-Administrator eine Reihe an <strong>Standardräumen</strong> importieren, die nach dem Einlesen allen Dozenten bei der Auswahl der Prüfungsräume als mögliche Räume zur Verfügung stehen. <br><br>
...
Das Datum und die Uhrzeit der Prüfung sollten hier also gesetzt werden, um die Prüfungsorganisation in {$a} sinnvoll nutzen zu können.';
$string['viewparticipants_help'] = 'Auf dieser Seite können alle zur Prüfung hinzugefügten <strong>Prüfungsteilnehmer/innen</strong> und Informationen wie deren Profil, Matrikelnummer sowie die ihnen gegebenenfalls in {$a} zugewiesenen Gruppen angesehen werden. <br /> <br />
...
Durch den Button "Zu Gruppe konvertieren" können schließlich einzelne oder alle hier aufgeführten Teilnehmer/innen in eine {$a} Gruppe umgewandelt werden.';
$string['converttogroup_help'] = 'Hier können ausgewählte oder alle importierten Prüfungsteilnehmer/innen in eine {$a} Gruppe umgewandelt werden. <br /> <br />
Dazu kann entweder für die Erstellung einer neuen {$a} Gruppe deren Name und optional auch Beschreibung in die entsprechenden Formularfelder eingetragen oder aber eine bereits bestehende Gruppe ausgewählt werden. Dann können im unteren Abschnitt alle gewünschten Teilnehmer/innen ausgewählt und danach durch einen Klick auf den Button "Zu Gruppe konvertieren" in eine Gruppe umgewandelt werden.<br /> <br />
Prüfungsteilnehmer/innen, die nicht zum Kurs gehören oder kein {$a} Benutzerkonto haben werden (falls vorhanden) in einem separaten Abschnitt angezeigt und können nicht zu einer Gruppe hinzugefügt werden. <br /> <br />
...
<li><strong>Neu eingelesen (kein/e Kursteilnehmer/in)</strong>: Der zu dieser Matrikelnummer gehörende Studierende ist nicht Teil des {$a}-Kurses. Er oder sie kann problemlos als Prüfungsteilnehmer/in importiert werden. Da er jedoch nicht die Teilnehmeransicht des Plugins ansehen kann muss er, um auszuschliessen dass hier ein Fehler vorliegt, durch Setzen des Hakens manuell ausgewählt werden.</li>
<li><strong>Neu eingelesen (ohne {$a} Benutzerkonto)</strong>: Der zu dieser Matrikelnummer gehörende Studierende hat noch keinen Account in {$a}. Dies kann etwa geschehen, wenn er sich noch nie in {$a} angemeldet hat. Der oder die Studierende kann zwar als Prüfungsteilnehmer/innen importiert werden, jedoch kann er dann nicht die Teilnehmeransicht der Prüfungsorganisation betrachten und Sie können ihn auch nicht über die Benachrichtigungssfunktion der Prüfungsorganisation erreichen. Deshalb müssen Sie diesen Studierenden hier manuell anhaken.</li>
...
$string['addcourseparticipants_help'] = 'Hier können alle im {$a} Kurs eingeschriebenen <strong>Kursteilnehmer/innen</strong> als Prüfungsteilnehmer/innen importiert werden. <br><br>
...
Diese bekommen die Nachricht direkt nach dem Abschicken sowohl als {$a}-Benachrichtigung als auch als E-Mail an ihren universitären E-Mail-Account weitergeleitet und können so zum Beispiel einfach auf kurzfristige Änderungen (etwa der Prüfungszeiten oder -Räume) aufmerksam gemacht werden. <br /> <br />
Falls Sie Teilnehmer/innen zur Prüfung hinzugefügt haben, die noch kein {$a}-Benutzerkonto haben, wird dies im Folgenden angezeigt. Da diese Teilnehmer/innen die hier geschriebene Nachricht nicht automatisch erhalten werden müssen Sie sie stattdessen manuell per E-Mail anschreiben. Dies können Sie zum Beispiel nach einem Klick auf den Button "Email schreiben", der ihren E-Mail-Client öffnet und die Mailadressen der entsprechenden Teilnehmer/innen einträgt, tun. <br /> <br />
...
1. Bewertungsexport aus {$a}: Haben Ihre Studierenden ihre Übungszettel über die {$a} Aufgabenabgabe abgegeben und wurden diese dort korrigiert und bewertet sollte hier der Bewertungsexport aus {$a} ausgewählt werden, da auf diese Weise sämtliche Bonuspunkte für den kompletten {$a} Kurs unkompliziert eingelesen werden können.<br>
Dazu müssen die Bewertungen aus dem {$a} Kurs zunächst wie <a href="https://docs.moodle.org/de/Bewertungen_exportieren" class="alert-link" target="_blank">hier</a> beschrieben exportiert werden. Danach müssen Sie die exportierte Datei einmal öffnen und nachsehen, in welcher Spalte die Punkte eingetragen sind. Die Bezeichnung der Spalte muss dann im dafür vorgesehenen Feld im unteren Abschnitt eingetragen werden.<br><br>
2. Individuell: Falls Sie ihre Bonuspunkte nicht über die {$a} Aufgabenabgabe verwaltet haben können Sie alternativ den Modus des individuellen Importes auswählen. Für diesen brauchen Sie eine Excel-Datei, bei der für jeden betroffenen Teilnehmenden in einer eigenen Zeile entweder die in {$a} hinterlegte Email-Adresse oder aber die Matrikelnummer in einer und die erreichte Punktzahl in einer anderen Spalte steht. Die Bezeichnung sowohl der Spalte, in der die Benutzerindentfikatoren aller Studierenden steht als auch die der Spalte, die alle Bonuspunktzahlen enthält müssen dann in den entsprechenden Feldern im unteren Abschnitt angegeben werden. <br><br>
...
$string['ldap_field_map_username_help'] = 'Hier muss die Bezeichnung des Feldes im LDAP-System eingetragen werden, in welchem der Benutzername des Teilnehmers steht. Dieser Benutzername muss mit dem Benutzernamen des Teilnehmers in {$a} übereinstimmen. Wird dieses Feld leer gelassen nutzt das Plugin bei aktiviertem LDAP den Wert "field_map_idnumber" aus den globalen Einstellungen des Authentifizierungsplugins LDAP-Server ("auth_ldap"). Ist keines dieser beiden Felder ausgefüllt können die vom LDAP abhängigen Pluginfunktionen nicht genutzt werden.';
