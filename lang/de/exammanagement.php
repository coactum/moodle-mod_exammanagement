...
$string['not_all_places_assigned_yet'] = 'Einigen Teilnehmenden müssen somit noch Sitzplätze zugewiesen werden, bevor mit den weiteren Arbeitsschritten fortgefahren werden kann.';
...
$string['bonussteps_set'] = 'Es wurden bisher Bonusnotenschritte für <strong>{$a->bonuscount} / {$a->participantscount}</strong> Teilnehmende importiert.';
...
Als Administrator können Sie hier auf Wunsch der oder des Lehrenden das Passwort der Prüfungsorganisation zurücksetzen. Alle Lehrenden des {$a->systemname}-Kurses werden darüber per {$a->systemname}-Nachricht benachrichtigt.';
$string['helptext_configurePassword'] = 'Auf dieser Seite kann ein <strong>Passwort</strong> für die Prüfungsorganisation gesetzt oder geändert werden. Dieses muss ab dann von jeder oder jedem Lehrenden des {$a->systemname}-Kurses eingegeben werden, um auf die Inhalte der Prüfungsorganisation zugreifen zu können. <br><br>
...
$string['helptext_addDefaultRooms']= 'Hier können Sie als {$a->systemname}-Administrator eine Reihe an <strong>Standardräumen</strong> importieren, die nach dem Einlesen allen Dozenten bei der Auswahl der Prüfungsräume als mögliche Räume zur Verfügung stehen. <br><br>
...
Das Datum und die Uhrzeit der Prüfung sollten hier also gesetzt werden, um die Prüfungsorganisation in {$a->systemname} sinnvoll nutzen zu können.';
$string['helptext_viewParticipants']= 'Auf dieser Seite können alle zur Prüfung hinzugefügten <strong>Prüfungsteilnehmer</strong> und Informationen wie deren Profil, Matrikelnummer sowie die ihnen gegebenenfalls in {$a->systemname} zugewiesenen Gruppen angesehen werden. <br /> <br />
...
<li><strong>Neu eingelesen (kein Kursteilnehmer)</strong>: Der zu dieser Matrikelnummer gehörende Studierende ist nicht Teil des {$a->systemname}-Kurses. Er kann problemlos als Prüfungsteilnehmer importiert werden. Da er jedoch nicht die Teilnehmeransicht des Plugins ansehen kann muss er, um auszuschliessen dass hier ein Fehler vorliegt, durch Setzen des Hakens manuell ausgewählt werden.</li>
<li><strong>Neu eingelesen (ohne {$a->systemname} Benutzerkonto)</strong>: Der zu dieser Matrikelnummer gehörende Studierende hat noch keinen Account in {$a->systemname}. Dies kann etwa geschehen, wenn er sich noch nie in {$a->systemname} angemeldet hat. Der oder die Studierende kann zwar als Prüfungsteilnehmer importiert werden, jedoch kann er dann nicht die Teilnehmeransicht der Prüfungsorganisation betrachten und Sie können ihn auch nicht über die Benachrichtigungssfunktion der Prüfungsorganisation erreichen. Deshalb müssen Sie diesen Studierenden hier manuell anhaken.</li>
...
$string['helptext_addCourseParticipants']= 'Hier können alle im {$a->systemname} Kurs eingeschriebenen <strong>Kursteilnehmerinnen</strong> als Prüfungsteilnehmer importiert werden. <br><br>
...
Diese bekommen die Nachricht direkt nach dem Abschicken sowohl als {$a->systemname}-Benachrichtigung als auch als E-Mail an ihren universitären E-Mail-Account weitergeleitet und können so zum Beispiel einfach auf kurzfristige Änderungen (etwa der Prüfungszeiten oder -Räume) aufmerksam gemacht werden. <br /> <br />
Falls Sie Teilnehmerinnen zur Prüfung hinzugefügt haben, die noch kein {$a->systemname}-Benutzerkonto haben, wird dies im Folgenden angezeigt. Da diese Teilnehmerinnen die hier geschriebene Nachricht nicht automatisch erhalten werden müssen Sie sie stattdessen manuell per E-Mail anschreiben. Dies können Sie zum Beispiel nach einem Klick auf den Button "Email schreiben", der ihren E-Mail-Client öffnet und die Mailadressen der entsprechenden Teilnehmer einträgt, tun. <br /> <br />
