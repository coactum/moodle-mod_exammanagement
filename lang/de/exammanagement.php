...
$string['export_grades_help'] = 'Hier können die von den KursteilnehmerInnen errungenen Bewertungen (als Punkte oder Noten) zuerst importiert, bei Bedarf in Noten umgerechnet und dann für das Prüfungsamt (zu den jeweiligen Matrikelnummern zugeordnet) als Textdatei exportiert werden. <br><br>
Dazu müssen ... <br>
...
$string['participantsOverview_grades_help'] = 'In dieser <strong>Bewertungsübersicht</strong> können für sämtliche importierten TeilnehmerInnen deren Bewertungspunkte und die daraus gegebenenfalls berechneten Noten angesehen und bearbeitet werden.<br><br>
Nach einem Klick auf den Button "Bewertungen bearbeiten" können für jeden Teilnehmenden manuell Bewertungen (sowohl als Punkt oder Notenwert) eingetragen oder aber bereits eingetragene Bewertungen bearbeitet werden. Mithilfe der Tabulator-Taste kann dabei zwischen den einzelnen TeilnehmerInnen gewechselt werden, während ein Klick auf den entsprechenden Button genau wie das Drücken der Enter-Taste alle vorgenommenen Änderungen speichert.<br><br>
...
$string['exam_participants_set'] = '<strong>{$a}</strong> Teilnehmerinnen und Teilnehmer sind zur Prüfung angemeldet.';
...
$string['message_to_participants_str'] = 'Hier können Nachrichten als interne Mitteilungen sowie Emails an alle zur Prüfung hinzugefügten Teilnehmerinnen und Teilnehmer versendet werden.';
...
$string['add_examresults_barcode'] = 'Ergebnisse mit Barcode-Scanner eintragen';
...
$string['groupmessages_text'] = 'Der unten eingegebene Text wird <strong>{$a->participantscount}</strong> zur Prüfung hinzugefügten Teilnehmern als {$a->systemname}-Benachrichtigung sowie als Email zugeschickt.';
$string['groupmessages_warning'] = '<strong>Achtung: {$a->participantscount}</strong> Prüfungsteilnehmer besitzen kein {$a->systemname}-Benutzerkonto und werden diese Nachricht deshalb nicht automatisch erhalten. Kontaktieren Sie diese Teilnehmerinnen deshalb am besten manuell per E-Mail durch einen Klick auf den folgenden Button:';
...
<ul><li>Um die weiteren Funktionen der '. get_config('mod_exammanagement', 'moodlesystemname').' Prüfungsorganisation nutzen zu können muss hier mindestens ein Raum als Prüfungsraum ausgewählt werden. Zudem müssen die gewählten Räume mindestens so viele Sitzplätze bieten, wie TeilnehmerInnen an der Prüfung teilnehmen sollen.</li>
...
Dazu kann entweder für die Erstellung einer neuen '.get_config('mod_exammanagement', 'moodlesystemname').' Gruppe deren Name und optional auch Beschreibung in die entsprechenden Formularfelder eingetragen oder aber eine bereits bestehende Gruppe ausgewählt werden. Dann können im unteren Abschnitt alle gewünschten Teilnehmer ausgewählt und danach durch einen Klick auf den Button "Zu Gruppe konvertieren" in eine Gruppe umgewandelt werden.<br /> <br />
PrüfungsteilnehmerInnen, die nicht zum Kurs gehören oder kein '.get_config('mod_exammanagement', 'moodlesystemname').' Benutzerkonto haben werden (falls vorhanden) in einem separaten Abschnitt angezeigt und können nicht zu einer Gruppe hinzugefügt werden. <br /> <br />
...
<strong>Achtung:</strong> Wird diese Variante des Teilnehmerimportes gewählt werden die Ergebnisse aller so hinzugefügten Teilnehmer später in einer einzelnen separaten Liste für das Prüfungsamt exportiert, wodurch der Reimport gegebenenfalls schwierig wird. Wenn Sie vorhaben, die Prüfungsergebnisse reimportieren zu lassen, sollten Sie die Teilnehmer lieber mithilfe der entsprechenden Teilnehmerlisten zur Prüfung hinzufügen.';
...
$string['assignPlaces_help']= 'Auf dieser Seite können allen PrüfungsteilnehmerInnen <strong>Sitzplätze</strong> für die Klausur zugewiesen werden. Diese können später in entsprechenden Dokumenten wie etwa den Sitzplänen oder Prüfungsetiketten exportiert werden.<br><br>
Sitzplätze können dabei entweder manuell oder aber automatisiert zugewiesen werden.<br /> <br />
Für die <strong>manuelle Zuweisung</strong> muss zunächst der entsprechende Button angeklickt werden. Danach kann in der Tabelle jedem Teilnehmenden manuell ein Sitzplatz in einem Raum zugewiesen werden. Die für jeden Raum verfügbaren Sitzplätze werden dabei angezeigt. Nach einem Klick auf den Button "Sitzplätze manuell zuweisen" werden sämtliche Änderungen bei Teilnehmern, denen sowohl ein Raum als auch ein Sitzplatz zugewiesen wurde gespeichert.<br /> <br />
...
Nach einem Klick auf den Button "Sitzplätze automatisch zuweisen" werden dann die Sitzplätze gemäß den gewählten Einstellungen zugewiesen. Durch Setzen des entsprechenden Hakens kann dabei festgelegt werden, dass bestehende Zuweisungen behalten und lediglich noch nicht zugewiesene Sitzplätze an TeilnehmerInnen ohne Platz zugewiesen werden. Andernfalls werden bestehende Zuweisungen komplett überschrieben. Bereits bestehende Zuweisungen lassen sich zudem über den entsprechenden Button komplett zurücksetzen.<br> <br />
<strong>Hinweis:</strong> Es ist so auch möglich, zunächst ausgewählten Teilnehmern bestimmte Plätze (etwa in den ersten Reihen) zuzuweisen und danach alle restlichen Plätze automatisch zuweisen zu lassen.';
...
$string['participantsOverview_help']= 'In dieser <strong>Teilnehmer- und Ergebnisübersicht</strong> können, sobald Prüfungsteilnehmerinnen importiert wurden, alle Informationen zu diesen angesehen und all ihre Prüfungsergebnisse eingetragen sowie bearbeitet werden. <br><br>
