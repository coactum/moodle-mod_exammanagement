...
/**
 * Allows teachers to configure the gradingscale for an exammanagement.
 *
...
// Instantiate form.
require_once($CFG->dirroot . '/mod/exammanagement/classes/forms/configuregradingscale_form.php');
$mform = new mod_exammanagement_configuregradingscale_form(null, ['id' => $id, 'e' => $e, 'moduleinstance' => $moduleinstance]);
...
if ($mform->is_cancelled()) { // Handle form cancel operation, if cancel button is present on form.
    ...
    $title = get_string('configuregradingscale', 'mod_exammanagement');

    $PAGE->set_url('/mod/exammanagement/configuregradingscale.php', ['id' => $id]);
    ...
    // Output description.
    echo '<p>' . get_string("configure_gradingscale_text", "mod_exammanagement") . '</p>';
